...
export default function NdaRequestForm({ labels, locale, whatsappUrl, telegramUrl }: Props) {
  ...
  if (status === 'success') {
    return (
      <div className="bg-card-dark border border-gold/60 p-8 md:p-10">
        <div className="serif text-3xl md:text-4xl text-gold mb-4">{labels.success.title}</div>
        <p className="text-sm md:text-base text-ink-white/85 leading-relaxed">{labels.success.body}</p>
        <div className="mt-6 pt-6 border-t border-line-dark/60">
  ...
  const inp =
    'w-full bg-transparent border border-line-dark px-4 py-3 text-sm md:text-base text-ink-white placeholder-ink-mutedDk focus:border-gold focus:outline-none transition-colors';
  const lbl = 'eyebrow text-ink-mutedDk mb-2 block';
  const err = 'text-xs text-neg-strong mt-1.5 block';
  ...
  return (
    <div className="flex flex-col gap-5">
      <form
        ...
        noValidate
        className="bg-card-dark p-6 md:p-8 border border-line-dark/60 flex flex-col gap-5"
      >
        {/* Honeypot */}
        ...
        {status === 'error' && (
          <div className="bg-neg-strong/15 border border-neg-strong/40 px-4 py-3 text-sm text-ink-white">
            {labels.error}
          </div>
        )}
        ...
        <label className="flex items-start gap-3 text-xs md:text-sm text-ink-mutedDk cursor-pointer">
          <input
            type="checkbox"
      ...
      <div className="flex items-center gap-4 my-1">
        <div className="flex-1 h-px bg-line-dark/60"></div>
        <span className="text-[10px] tracking-[0.2em] uppercase text-ink-mutedDk">{labels.altLabel}</span>
        <div className="flex-1 h-px bg-line-dark/60"></div>
      </div>
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <a
        ...
        className="flex items-center justify-center gap-3 px-4 py-3 border border-line-dark hover:border-gold/60 text-sm md:text-base text-ink-white transition-colors group"
      >
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" className="text-[#25D366] group-hover:text-gold transition-colors">
      ...
      <a
        ...
        className="flex items-center justify-center gap-3 px-4 py-3 border border-line-dark hover:border-gold/60 text-sm md:text-base text-ink-white transition-colors group"
      >
        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor" className="text-[#229ED9] group-hover:text-gold transition-colors">
